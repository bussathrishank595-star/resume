...
function InputField({ label, name, value, onChange, type = "text", placeholder, min, step }) {
  return (
...
function App() {
  ...
  const aiBenchmark = analysisResult?.ai_benchmark ?? null;
  const comparableProducts = Array.isArray(aiBenchmark?.comparable_products)
    ? aiBenchmark.comparable_products
    : [];
  const aiImageAnalysis = analysisResult?.ai_image_analysis ?? null;
  const imageVisualFlags = Array.isArray(aiImageAnalysis?.visual_flags)
      ...
      <section className="mx-auto w-full max-w-5xl space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-4">
            <TrustMeter score={analysisResult?.confidence_score ?? 0} />
